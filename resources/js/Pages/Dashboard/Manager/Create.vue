...
export default {
    ...
    props: { shops: { type: Array, default: [] } },
    data() {
        return {
            item: {
                name: "",
    ...
    methods: {
        submit() {
            axios
                .post(route("managers.store"), this.item)
                .then((res) => {
                    Swal.fire({
                    ...
                    router.visit(route("managers.index"), { replace: true });
                })
                .catch((err) => {
...
<template>
    <Head title="Добавить менеджера" />
    <DashboardLayout>
        <div class="mb-6">
            <h2 class="font-bold text-xl">Добавить менеджера</h2>
        </div>
